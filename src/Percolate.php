...
class Percolate
{
     ...
     * @var array
     */
    protected array $options = [self::OPTION_DOCS_JSON => 1];
     ...
     * @throws SphinxQLException
     */
    public function insert(string $query, bool $noEscape = false): static
    {
        $this->clear();
     ...
     * @throws SphinxQLException
     */
    public function execute(): Drivers\ResultSetInterface
    {
     ...
     * @throws SphinxQLException
     */
    private function setOption(string $key, int $value): static
    {
        $value = intval($value);
        if (!in_array($key, [
     ...
     * @throws SphinxQLException
     */
    public function documents($documents): static
    {
        if (empty($documents)) {
            ...
            if (is_array($this->documents)) {
                ...
                if (!empty($this->documents[0]) && !is_array($this->documents[0]) &&
                    ...
                    if (!$this->isAssocArray($this->documents)) {
                        ...
                        // the document is an associative array like ['foo' => 'bar']
                        $this->options[self::OPTION_DOCS_JSON] = 1;
     ...
     * @return bool|string
     */
    private function prepareFromJson($data): bool|string
    {
        if (is_array($data)) {
            if (is_array($data[0])) {
                return false;
